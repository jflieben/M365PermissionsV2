# M365PermissionsV2 — Module entry point
...
function Get-M365Engine {
    if ($null -eq $script:Engine) {
        $dbFolder = Join-Path ([Environment]::GetFolderPath('ApplicationData')) 'LiebenConsultancy' 'M365PermissionsV2'
...
if ([Environment]::UserInteractive -and -not [Environment]::GetCommandLineArgs().Contains('-NonInteractive') -and -not $env:M365_NO_AUTOSTART) {
    try {
        ...
        Write-Host "M365PermissionsV2 GUI started at http://localhost:$port" -ForegroundColor Cyan
    }
    catch {
